...
class PyMorelInputData():
    ...
    def set_dataframes(self):
        ...
        self.t = pandas.DataFrame(self.inputdata['t_data'])         # Technology data
        self.te = pandas.DataFrame(self.inputdata['te_data'])       # Technonology x energy carrier data
        self.ty = pandas.DataFrame(self.inputdata['ty_data'])       # Technology x year data
        ...
        self.t = pandas.merge(self.t,self.ty['tech'], on='tech')    # merge will drop t rows that have no tech in ty
        self.te = pandas.merge(self.te,self.ty['tech'], on='tech')  # merge will drop te rows that have no tech in ty
    ...
    def set_sets(self):
        ...
        self.sets = {
            ...
            'T':  self.t['tech'].to_list(),                          # All active technologies
        ...
        self.subsets = {
            ...
            'TC': self.ty['tech'][self.ty.maxC > 0].to_list(),
            # Technology subsets by role
            'TT':  self.t['tech'][self.t.role == 'tfrm'].to_list(),
            'TX':  self.t['tech'][self.t.role == 'trms'].to_list(),
            'TS':  self.t['tech'][self.t.role == 'stor'].to_list(),
    ...
    def set_para_hourly(self):
        ...
        twh = pandas.merge(self.t[['tech','role','vAva']], wh, on='vAva').drop(['vAva'], axis=1)
    ...
    def set_para_yearly(self):
        ...
        ty = pandas.merge(ty, self.t[['tech','role','cstC']], on='tech')
